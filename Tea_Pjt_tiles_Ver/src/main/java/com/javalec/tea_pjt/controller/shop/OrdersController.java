...
public class OrdersController {
	...
	public String orderedpage(HttpSession session,Model model){
		...
		ArrayList<OrdersDTO> orders_detail = new ArrayList<>();
				
		orders_detail = (ArrayList<OrdersDTO>) orderService.orders_detail(userid);
		
		for(int i=0; i<orders_detail.size(); i++){
			int orders_id = orders_detail.get(i).getOrder_id();
			ArrayList<OrdersDTO> select_orders = (ArrayList<OrdersDTO>) orderService.select_orders(orders_id);
			model.addAttribute("select_orders"+i,select_orders);
		}
		
		model.addAttribute("orders_detail", orders_detail);
